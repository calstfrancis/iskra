...
use super::calendar::{
    advent_sunday, is_sunday, lectionary_year, nearest_sunday, sunday_on_or_before,
};
use super::rcl_complementary_data;
use super::{LiturgicalInfo, RclTrack};
...
fn lookup(key_year: &str, sunday_id: &str, track: RclTrack) -> Option<Readings> {
    let base = READINGS
        .iter()
    ...
    if track != RclTrack::Complementary || !sunday_id.starts_with("Proper") {
        return base;
    }
    // Track 2 only swaps OT + Psalm; Epistle/Gospel are shared with Track 1.
    rcl_complementary_data::COMPLEMENTARY
        .iter()
        .find(|&&(yk, sid, _)| yk == key_year && sid == sunday_id)
        .map(|&(_, _, (ot, psalm))| {
            let (_, _, epistle, gospel) = base.unwrap_or(("—", "—", "—", "—"));
...
fn result_for(
    ...
    let r = lookup(yk, sunday_id, track);
...
/// two apps agree on any given Sunday (Track/Semicontinuous data only —
/// Rubric has no Complementary track).
pub fn get_liturgical_info(d: NaiveDate, track: RclTrack) -> LiturgicalInfo {
    use chrono::Datelike;
    let year = d.year();
    let lec_year = lectionary_year(d);

    let e = super::calendar::easter(year);
    let prev_e = super::calendar::easter(year - 1);
    ...
    // ── Special fixed days ──────────────────────────────────────────────
    if d == good_fri {
        return result_for(lec_year, "Good Friday", "Good Friday", "GoodFriday", None, track);
    }
    if d == holy_thu {
        return result_for(
            lec_year,
    ...
    if d == ash_wed {
        return result_for(lec_year, "Lent", "Ash Wednesday", "AshWednesday", None, track);
    }
    ...
    if xmas <= d && d <= dec31 {
        if d == xmas {
            ...
            return LiturgicalInfo {
                ...
                colour_hex: colour_hex.to_string(),
                readings: vec![
                    ("OT".to_string(), "Isa 9:2–7".to_string()),
                    ("Psalm".to_string(), "Ps 96".to_string()),
                    ("Epistle".to_string(), "Titus 2:11–14".to_string()),
                    ("Gospel".to_string(), "Luke 2:1–14".to_string()),
                ],
                found: true,
            };
        }
        let days_to_next_sun = (6 - super::calendar::python_weekday(d)) % 7;
    ...
    if jan1 <= d && d <= jan5 && is_sunday(d) {
        return result_for(lec_year, "Christmas", "Christmas 2", "Christmas2", None, track);
    }
    ...
    if d == epiphany {
        ...
        return LiturgicalInfo {
            ...
            colour_hex: colour_hex.to_string(),
            readings: vec![
                ("OT".to_string(), "Isa 60:1–6".to_string()),
                ("Psalm".to_string(), "Ps 72:1–7, 10–14".to_string()),
                ("Epistle".to_string(), "Eph 3:1–12".to_string()),
                ("Gospel".to_string(), "Matt 2:1–12".to_string()),
            ],
            found: true,
        };
    }
    ...
    if d == ascension {
        return result_for(lec_year, "Easter", "Ascension of the Lord", "Ascension", None, track);
    }
...
mod tests {
    ...
    fn info(d: NaiveDate) -> LiturgicalInfo {
        get_liturgical_info(d, RclTrack::Semicontinuous)
    }
    ...
    fn ash_wednesday_2024_season() {
        assert_eq!(info(d(2024, 2, 14)).season, "Lent");
    }

    #[test]
    fn ash_wednesday_2024_week() {
        assert_eq!(info(d(2024, 2, 14)).week, "Ash Wednesday");
    }

    #[test]
    fn lent_has_readings() {
        let i = info(d(2024, 3, 10)); // Lent 4
        assert!(i.found);
        assert_ne!(i.gospel(), "—");
    }
    ...
    fn palm_sunday_2024_season() {
        assert_eq!(info(d(2024, 3, 24)).season, "Palm Sunday");
    }

    #[test]
    fn good_friday_2024() {
        let i = info(d(2024, 3, 29));
        assert!(i.found);
        assert_eq!(i.season, "Good Friday");
    }
    ...
    fn easter_sunday_2024_season() {
        assert_eq!(info(d(2024, 3, 31)).season, "Easter");
    }

    #[test]
    fn easter_sunday_2024_week() {
        assert!(info(d(2024, 3, 31)).week.contains("Easter Sunday"));
    }

    #[test]
    fn easter_has_readings() {
        let i = info(d(2024, 3, 31));
        assert!(i.found);
        assert_ne!(i.gospel(), "—");
    }
    ...
    fn advent_1_2024_season() {
        assert_eq!(info(d(2024, 12, 1)).season, "Advent");
    }

    #[test]
    fn advent_has_readings() {
        let i = info(d(2024, 12, 1));
        assert!(i.found);
        assert_ne!(i.gospel(), "—");
    }
    ...
    #[test]
    fn christmas_2024() {
        let i = info(d(2024, 12, 25));
        assert_eq!(i.season, "Christmas");
        assert!(i.found);
    }
    ...
    fn ordinary_time_season() {
        assert_eq!(info(d(2024, 7, 14)).season, "Ordinary");
    }

    #[test]
    fn ordinary_time_has_proper() {
        assert!(info(d(2024, 7, 14)).week.contains("Ordinary"));
    }

    #[test]
    fn ordinary_time_has_readings() {
        assert!(info(d(2024, 7, 14)).found);
    }

    #[test]
    fn complementary_track_changes_ot_but_not_gospel() {
        let semi = get_liturgical_info(d(2024, 6, 1), RclTrack::Semicontinuous); // Proper 4
        let comp = get_liturgical_info(d(2024, 6, 1), RclTrack::Complementary);
        assert_eq!(semi.gospel(), comp.gospel());
    }
    ...
    fn advent_colour_purple() {
        let colour = info(d(2024, 12, 1)).colour.to_lowercase();
        assert!(colour == "purple" || colour == "violet");
    }

    #[test]
    fn easter_colour_white() {
        let colour = info(d(2024, 3, 31)).colour.to_lowercase();
        assert!(colour.contains("white"));
    }

    #[test]
    fn ordinary_colour_green() {
        assert_eq!(info(d(2024, 7, 14)).colour.to_lowercase(), "green");
    }

    #[test]
    fn lent_colour_purple() {
        let colour = info(d(2024, 3, 10)).colour.to_lowercase();
        assert!(colour == "purple" || colour == "violet");
    }
    ...
    #[test]
    fn year_in_info() {
        let year = info(d(2024, 7, 14)).year;
        assert!(year == "A" || year == "B" || year == "C");
    }

    #[test]
    fn year_b_easter_2024() {
        assert_eq!(info(d(2024, 3, 31)).year, "B");
    }
    ...
    fn epiphany_season_weekdays_do_not_crash() {
        ...
        for i in 0..90 {
            let _ = info(start + chrono::Duration::days(i));
        }
    }

    #[test]
    fn january_weekday_no_crash_specific() {
        let i = info(d(2026, 1, 13));
        assert!(!i.season.is_empty());
    }
    ...
    fn golden_year_all_sundays_resolve_without_panic_and_have_valid_year() {
        ...
        while day < end {
            if is_sunday(day) {
                let i = info(day);
                assert!(matches!(i.year.as_str(), "A" | "B" | "C"));
            }
